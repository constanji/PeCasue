...
class X2TConverter {
    ...
    private get SCRIPT_PATH(): string {
        if (typeof window !== 'undefined') {
            return new URL('wasm/x2t/x2t.js', `${window.location.origin}/`).href
        }
        const base = import.meta.env.BASE_URL || '/'
        const normalized = base.endsWith('/') ? base : `${base}/`
        return `${normalized}wasm/x2t/x2t.js`
    }
    ...
    private executeConversion(paramsPath: string): void {
        ...
        if (result !== 0) {
            const hint =
                result === 88
                    ? '（常见于超大 xlsx：浏览器 WASM 内存或转换上限；请下载后用 Excel 打开）'
                    : ''
  ...
  ${additionalParams}
            ...
            files
                ...
                .forEach((file) => {
                    try {
                        ...
                        const blob = new Blob([toArrayBuffer(fileData)])
    ...
    async convertDocument(file: File): Promise<ConversionResult> {
        ...
        try {
            ...
            const hint = isAbort
                ? ' 常见于文件过大或浏览器内存不足。请下载 Excel 后本地打开；若本次运行生成了「三表汇总」轻量文件，可用其在线预览。'
                : ''
            throw new Error(`Document conversion failed: ${raw}.${hint}`)
        }
    }
