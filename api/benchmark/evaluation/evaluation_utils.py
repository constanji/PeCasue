...
# PyMySQL  1.1.1
def connect_mysql(database_name=None):
    # Open database connection
    # Connect to the database
    # 从db_path中提取数据库名称，例如：card_games/card_games.sqlite -> card_games
    if database_name is None:
        database_name = "BIRD"  # 默认数据库
    ...
    db = pymysql.connect(
        ...
        port=3306,
        # unix_socket="/tmp/mysql.sock",  # macOS 如果使用socket连接，取消注释此行并注释port
    )
    return db
...
def connect_db(sql_dialect, db_path):
    if sql_dialect == "SQLite":
        ...
        # 从db_path中提取数据库名称
        # db_path格式: ../mini_dev_data/dev_databases/card_games/card_games.sqlite
        # 或: card_games/card_games.sqlite
        if db_path:
            # 提取数据库名称（最后一个路径段的名称，去掉.sqlite后缀）
            db_name = db_path.split("/")[-1].replace(".sqlite", "")
        else:
            db_name = None
        conn = connect_mysql(database_name=db_name)
